...
def group_by_centroid(restaurants, centroids):
    ...
    "*** REPLACE THIS LINE ***"
    # END Question 4
    return group_by_first([[find_closest(restaurant_location(k),centroids),k] for k in restaurants])
...
def find_centroid(cluster):
    ...
    k=[]
    for i in cluster:
        k+= [restaurant_location(i)]
    sum_lat = [x for [x,y] in k]
    sum_lon = [y for [x,y] in k]
    return [mean(sum_lat),mean(sum_lon)]

    # END Question 5


def k_means(restaurants, k, max_updates=100): # group restaurnats into clusters
